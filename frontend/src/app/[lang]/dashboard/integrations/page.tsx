...
export default async function IntegrationsPage({ params }: { params: Promise<{ lang: 'pl' | 'en' }> }) {
    ...
    <div className="w-full p-6 sm:p-10 lg:p-14">
      <header className="mb-12 relative">
        <h1 className="text-4xl sm:text-5xl font-extrabold text-white tracking-tight mb-2">
          {dict.integrations?.title || "Integracje"}
        </h1>
      ...
      <IntegrationsClient dict={dict} token={token} profile={profile} lang={lang} />
    </div>
  );
}