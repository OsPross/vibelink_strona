...
import { Mail, Lock, User, Loader2, AlertCircle, ArrowLeft, PaintRoller, Sparkles, Terminal } from "lucide-react";

export default function RegisterClient({ lang, dict }: { lang: string; dict: any }) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Ten nick lub email jest już zajęty.");
  ...
  const handleOAuth = (provider: string) => {
    setIsLoading(true);
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000";
    // Przekierowanie na backend
    window.location.href = `${apiUrl}/auth/${provider}`;
  };

  return (
    <div className="fixed inset-0 w-full h-full flex flex-col lg:flex-row bg-[#030303] text-white selection:bg-emerald-500/30 overflow-hidden font-sans z-[9999]">
      ...
      <div className="hidden lg:flex w-1/2 h-full relative flex-col justify-between p-12 border-r border-white/5 overflow-hidden bg-[#030303]">
        <div className="absolute inset-0 z-0 pointer-events-none">
          <div className="absolute top-[-10%] right-[-10%] w-[40vw] h-[40vw] bg-emerald-600/20 blur-[130px] rounded-full mix-blend-screen" />
          <div className="absolute bottom-[-10%] left-[-10%] w-[40vw] h-[40vw] bg-cyan-600/20 blur-[130px] rounded-full mix-blend-screen" />
          ...
          <div className="relative w-full max-w-md h-64">
             <motion.div animate={{ y: [0, -15, 0] }} transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }} className="absolute top-0 left-0 w-72 bg-[#050505] border border-white/10 p-6 rounded-[2rem] shadow-2xl -rotate-3">
                <div className="flex items-center gap-4">
                   <div className="w-12 h-12 rounded-xl bg-emerald-500/20 flex items-center justify-center"><PaintRoller className="w-6 h-6 text-emerald-400"/></div>
             ...
             <motion.div animate={{ y: [0, 15, 0] }} transition={{ duration: 7, repeat: Infinity, ease: "easeInOut", delay: 1 }} className="absolute bottom-0 right-0 w-64 bg-white/5 backdrop-blur-xl border border-white/10 p-5 rounded-3xl shadow-2xl rotate-6">
                <div className="flex items-center gap-3">
                   <div className="w-10 h-10 rounded-xl bg-cyan-500/20 flex items-center justify-center"><Terminal className="w-5 h-5 text-cyan-400"/></div>
        ...
        <div className="relative z-10">
          <h2 className="text-4xl font-black tracking-tighter mb-4 leading-tight">Zajmij swoją <br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400">przestrzeń.</span></h2>
          <p className="text-zinc-400 text-lg max-w-sm">Zbuduj hub dla swojej społeczności w domenie vibelink.pl.</p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 h-full flex flex-col items-center justify-center p-6 sm:p-12 relative bg-[#080808]">
        <div className="absolute inset-0 lg:hidden pointer-events-none overflow-hidden">
           <div className="absolute top-0 left-0 w-full h-96 bg-emerald-600/10 blur-[100px]" />
        </div>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="w-full max-w-[420px] relative z-10">
          ...
          <div className="mb-8 text-center lg:text-left flex flex-col items-center lg:items-start">
            ...
            <h1 className="text-3xl sm:text-4xl font-black tracking-tight text-white mb-1">Zainicjuj Protokół</h1>
            <p className="text-zinc-400 text-sm">Zarezerwuj swój adres URL.</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1">Twój Link</label>
              <div className="flex items-center bg-white/[0.03] border border-white/10 rounded-xl focus-within:ring-1 focus-within:ring-emerald-500 transition-all overflow-hidden">
                <div className="pl-4 pr-2 py-3.5 text-zinc-500 border-r border-white/5 bg-white/[0.01] text-xs font-mono font-bold">
                  vibelink.pl/
                </div>
                <input type="text" value={username} onChange={(e) => setUsername(e.target.value.toLowerCase().replace(/\s/g, ''))} required placeholder="nick" className="w-full bg-transparent py-3.5 px-3 text-white text-sm outline-none font-mono font-bold" />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative group/input">
                 <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500 group-focus-within/input:text-emerald-400 transition-colors" />
                 <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required placeholder="hacker@vibelink.pl" className="w-full bg-white/[0.03] border border-white/10 py-3.5 pl-11 pr-4 rounded-xl text-white outline-none focus:ring-1 focus:ring-emerald-500 transition-all text-sm" />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative group/input">
                 <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500 group-focus-within/input:text-emerald-400 transition-colors" />
                 <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required placeholder="••••••••" className="w-full bg-white/[0.03] border border-white/10 py-3.5 pl-11 pr-4 rounded-xl text-white outline-none focus:ring-1 focus:ring-emerald-500 transition-all font-mono text-sm" />
              </div>
            </div>

            <button type="submit" disabled={isLoading} className="w-full bg-white hover:bg-zinc-200 text-black font-black text-base py-3.5 rounded-xl transition-all flex items-center justify-center gap-2 mt-2 shadow-xl active:scale-[0.98]">
              {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <>Utwórz profil <Sparkles className="w-4 h-4" /></>}
            </button>
          </form>
          ...
          <div className="mt-4 text-center lg:text-left border-t border-white/5 pt-4">
            <p className="text-zinc-500 text-xs font-medium">Masz już konto? <Link href={`/${lang}/login`} className="text-white font-bold hover:text-emerald-400 transition-colors">Zaloguj się</Link></p>
          </div>
        </motion.div>
      </div>
    </div>
